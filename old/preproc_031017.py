...
def dataformat(trainset, testset, limits, droplist=[]):
  ...
  limits: python dict
    ...
    'elder':  the limit age to define a passenger as an elder person.
  ...
  # add a 'Relatives' column to data
  ...
  for i in list(dataset['Cabin'].unique()):
    counts = dataset.loc[dataset['Cabin']==i,'Cabin'].head(trainsize).count()
  ...
  for title in garbage.keys():
    dataset.loc[(dataset['Name']==title) & (dataset['Age'].isnull()), 'Age'] = limits['elder']+1
  ...
  dataset.loc[dataset['Age']>limits['elder'], 'Age'] = 2
  dataset.loc[(dataset['Age']>=limits['child']) & (dataset['Age']<=limits['elder']), 'Age'] = 1
  ...
  dataset.loc[dataset['Relatives']>=limits['relatlimit'], 'Relatives'] = limits['relatlimit']
